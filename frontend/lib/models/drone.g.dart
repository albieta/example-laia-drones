...
class _$DroneCWProxyImpl implements _$DroneCWProxy {
  ...
  @override
  Drone ownerId(String? ownerId) => this(ownerId: ownerId);
...
class _DroneWidgetState extends State<DroneWidget> {
  ...
  final GlobalKey<StringWidgetState> nameWidgetKey =
      GlobalKey<StringWidgetState>();
  final GlobalKey<DronePilotFieldWidgetState> ownerIdWidgetKey =
      GlobalKey<DronePilotFieldWidgetState>();
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            DronePilotFieldWidget(
              key: ownerIdWidgetKey,
              fieldName: "Owner",
              fieldDescription: "This is the Drone Owner",
              editable: true,
              placeholder: "Select the Drone Owner",
...
class _DroneListViewState extends ConsumerState<DroneListView> {
  ...
  Widget build(BuildContext context) {
    ...
    Future<List<DronePilot>> fetchDronePilotList(List<String>? ids) async {
      if (ids == null || ids.isEmpty) {
        return [];
      }
      final nonEmptyIds = ids.where((id) => id.isNotEmpty).toList();
      List<DronePilot> dronepilotList = await Future.wait(
        nonEmptyIds.map((id) async {
          return await ref.read(getDronePilotProvider(id).future);
        }),
      );
      return dronepilotList;
    }
